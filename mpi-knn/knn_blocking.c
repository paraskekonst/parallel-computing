 ...
 for (int k=0; k<nproc-1; k++){
   for(int i =0; i<rows; i++){
    for (int j=0; j<dimensions; j++){
       if (k==0){
        MPI_Send( &(localdata[i][j]),1,  MPI_DOUBLE, to, 0, MPI_COMM_WORLD);
        MPI_Recv(&(rec[i][j]),1 , MPI_DOUBLE,from ,0 ,   MPI_COMM_WORLD, &status);
      }else{
        MPI_Send( &(send[i][j]),1,  MPI_DOUBLE, to, 0, MPI_COMM_WORLD);
        MPI_Recv(&(rec[i][j]),1 , MPI_DOUBLE,from ,0 ,   MPI_COMM_WORLD, &status);
        }
     }
    }
...
void knn (double **localdata,double ** values1,double **arrayofdistance,double **index ,int currentrank ,int rank )
{
 ...
 for(int i =0; i<rows; i++){
  for (int i1 = 0 ; i1<rows; i1 ++){
     if(i==i1 && rank==currentrank)continue;
      else{ 
